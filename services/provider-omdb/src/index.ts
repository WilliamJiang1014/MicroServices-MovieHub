...
const logger = new Logger('OMDb-Provider');

app.use(cors());
app.use(express.json());

const omdbClient = new OMDBClient(process.env.OMDB_API_KEY || '');

// Initialize Redis and Cache Manager
const redis = getRedisClient();
const cacheManager = new CacheManager(redis, {
  keyPrefix: 'provider:omdb',
  defaultTTL: 3600, // 1 hour for provider data
});

// Health check
app.get('/health', (req, res) => {
  res.json({ status: 'healthy', service: 'provider-omdb' });
});
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    if (cached) {
      logger.info(`Returning cached OMDb search for: ${params.query}`);
      return res.json(cached);
    }

    const result = await omdbClient.search(params);
...
app.get('/api/movie/:id', async (req, res) => {
  try {
    ...
    if (cached) {
      logger.info(`Returning cached OMDb movie: ${movieId}`);
      return res.json(cached);
    }

    const result = await omdbClient.getById(movieId);

    // Cache the result (longer TTL for movie details)
    await cacheManager.cacheProviderResponse(
      'omdb',
      'movie',
      { id: movieId },
...
// Get movie by external ID (with caching)
app.get('/api/movie/external/:type/:id', async (req, res) => {
  try {
    ...
    if (cached) {
      logger.info(`Returning cached OMDb external lookup: ${type}:${id}`);
      return res.json(cached);
    }

    const result = await omdbClient.getByExternalId(id, type);
...
app.listen(port, () => {
  logger.info(`OMDb Provider listening on port ${port}`);
});
